...
export const firebaseConfig = {
  apiKey: "AIzaSyBZGgixu_5JclBn3uAhJqUtp1owOFRYzbA",
  authDomain: "sajjat-ai.firebaseapp.com",
  databaseURL: "https://sajjat-ai-default-rtdb.asia-southeast1.firebasedatabase.app",
  projectId: "sajjat-ai",
  storageBucket: "sajjat-ai.firebasestorage.app",
  messagingSenderId: "116242667370",
  appId: "1:116242667370:web:fadc7a5445f8266588b713",
  measurementId: "G-2KW16MKXP6"
};
...
export async function registerUser(name: string, email: string, pass: string): Promise<UserProfile> {
  ...
  // Save profile to Realtime Database at users/{uid}/profile
...
export async function loginUser(email: string, pass: string): Promise<UserProfile> {
  ...
  if (snapshot.exists()) {
    ...
    await update(profileRef, { lastLoginAt: profileData.lastLoginAt });
  } else {
    profileData = {
      uid: user.uid,
      email: user.email || email,
...
export async function loginAdmin(email: string, pass: string): Promise<UserProfile> {
  ...
  // 1. Check if provided pass matches any Master Admin Passkey / PIN
...
export function subscribeToAllUsers(onUpdate: (users: any[]) => void) {
  ...
  return onValue(usersRef, (snapshot) => {
    if (snapshot.exists()) {
      ...
      Object.entries(data).forEach(([uid, userObj]: [string, any]) => {
    ...
    console.warn("Failed to subscribe to all users:", error);
    onUpdate([]);
  });
}
